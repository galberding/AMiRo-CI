...
class GccVersionChecker:
    ...
    def __init__(self) -> None:
        self.regex = re.compile(
            fr'gcc\sversion\s(?P<{Version.major.name}>\d+)\.(?P<{Version.minor.name}>\d+)\.(?P<{Version.fix.name}>\d+)'
        )
    ...
    def get_version_string(self) -> str:
        ...
        process = subprocess.run(
            ['arm-none-eabi-gcc', '-v'], capture_output=True
        )
    ...
    def check_version(self, version: tuple[int, int, int]):
        ...
        if version[Version.major.value] < 9:
            raise WrongGccVersion(
                *version,
                f'Major version too low requires at least 9 or higher!'
            )
