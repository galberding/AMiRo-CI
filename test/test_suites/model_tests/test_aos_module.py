...
class TestAosModel(unittest.TestCase):
    ...
    def test_module_with_options_is_resolved(self):
        result = SearchResult(
            [('USE_FPU', 'softfp'), ('USE_FPU_OPT', '-mfpu=fpv4-sp-d16')],
            MakeOption
        )
    ...
    def test_module_find_option_by_name_not_found_exception(self):
        self.assertRaises(
            OptionNotFoundException, self.aos_module.find_option_by_name,
            'USE_FPU'
        )
    ...
    def test_module_resolve_multiple_options(self):
        search_res = [
            (
                (
                    "USE_FANCY_EXCEPTIONS_STACK",
                    "-stack-usage=$(USE_EXCEPTIONS_STACKSIZE)"
                )
        ...
        self.assertTrue(self.aos_module.is_resolved())
        exc_option = self.aos_module.find_option_by_name(
            "USE_FANCY_EXCEPTIONS_STACK"
        )
    ...
    def test_module_cannot_resolve_all_options(self):
        ...
        self.search_results.append(
            ("USE_RANDOM_VALUE", "-set-seed=$(USE_UNKNOWN_OPTION)")
        )
    ...
    def test_module_resolve_multiple_args_with_same_sub_option(self):
        search_res = [
            (("USE_THIS_SUB", "42")), (("USE_CASE1", "-case1=$(USE_THIS_SUB)")),
            (("USE_CASE2", "-case2=$(USE_THIS_SUB)")),
            (("USE_CASE3", "-case3=$(USE_THIS_SUB)"))
        ]
        self.build_options_and_resolve_module(
            search_res, self.aos_module, opt_type=CfgOption
        )
        self.assertTrue(self.aos_module.is_resolved())
    ...
    def build_options_and_resolve_module(
        self,
        search_res,
        module: AosModule,
