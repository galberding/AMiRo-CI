...
from amirotest.tools import AosConfigFinder, CannotFindModuleError
...
class TestConfigPathFinder(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.aos_finder = AosConfigFinder(self.nucleo.path)


    def test_create_aos_connfig_finder(self):
        c_finder = AosConfigFinder(self.path_helper.get_aos_module_path())
        self.assertEqual(self.path_helper.aos_path, c_finder.aos_root)

    def test_create_finde_unknown_module_raise_exception(self):
        self.assertRaises(CannotFindModuleError, AosConfigFinder, Path("non/existent/"))
    ...
    def test_aos_get_makefile(self):
        self.assertEqual("Makefile", self.aos_finder.get_makefile().name)
